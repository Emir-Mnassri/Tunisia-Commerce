...
export default function Success() {
  ...
  // Could be either orderId (from cash_on_delivery) or payment_id (from Flouci)
  // For the mockup, we might just have an orderId
    ...
    <PageTransition className="bg-background py-16 md:py-24">
      <div className="container mx-auto px-4 md:px-6 max-w-3xl text-center">
        ...
        {isLoading ? (
          ...
          <div className="bg-background border border-border/60 text-left overflow-hidden mb-12">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-foreground mb-2">
                  {order.paymentMethod === 'cash_on_delivery' ? 'Paiement à la livraison' : 'Paiement en ligne (Flouci)'}
                </p>
                  ...
                  {order.items.map((item, idx) => (
                    <li key={idx} className="flex justify-between text-sm">
                      <span className="text-muted-foreground">
                        {item.quantity}x <span className="text-foreground font-medium">{item.productName}</span>
                      </span>
        ...
        <div className="flex justify-center">
          <Link href="/products">
            <Button size="lg" className="rounded-none h-14 px-8 text-base font-medium group" data-testid="button-continue-shopping">
